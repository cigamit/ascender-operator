...
After the GHA is complete, the final step is to run the [publish-to-operator-hub.sh](https://github.com/ctrliq/ascender-operator/blob/devel/hack/publish-to-operator-hub.sh) script, which will create a PR in the following repos to add the new ascender-operator bundle version to OperatorHub:
